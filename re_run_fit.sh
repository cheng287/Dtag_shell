...
DataParentPath1=$PWD   
#DataParentPath1=/workfs2/bes/liucheng/try/fit    
...
do
        ...
        if [ -d "$DataParentPath1/fit/mode_${decay_mode_points[$dm_i]}/failed" ]
        then
            echo "$DataParentPath1/fit/mode_${decay_mode_points[$dm_i]}/failed is ok"
        else
            echo "* * * * * * * * * * * *  $DataParentPath1/fit/mode_${decay_mode_points[$dm_i]}/failed is not exit ! ! !"
            let dm_i++
            continue
        fi


        cd ${DataParentPath1}/fit/mode_${decay_mode_points[$dm_i]}/failed
