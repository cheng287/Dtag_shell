...
do
    ...
    do
        ...
        if [ -d "../output/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}" ]              
        then
            echo "../output/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]} is ok"
        else
            mkdir -p ../output/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}
        fi
        ...
        then
            ...
            do
                ...
                do
                    ...
                    then 
                        if [ -e ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp ] 
                        then 
                           echo "Just remind: ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp already exists, and it will be update!"
                        fi 

                        #* * * *  The mass of Ds and D0,D+- are different, so we should use different fitting scripts.
                        if [ ${decay_mode_points[$dm_i]} -lt 400 ]
                        then
                            sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}/ ${mypath}/fitjob.head > ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp
                        else
                            sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}/ ${mypath}/Ds_fitjob.head > ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp
                        fi
                    ...
                    then 
                        ...
                        echo  "    chaindata->Add("\"${file_number[$xy-1]}\" ") ;" >>  ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp
                    fi 
                    ...
                    then 
                        ...
                        if [ ${decay_mode_points[$dm_i]} -lt 400 ]
                        then       
                            sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}/ ${mypath}/all_fitjob.tail >> ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp
                        else
                            sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}/ ${mypath}/all_Dsfitjob.tail >> ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp
                        fi

                        # sed -i "s#CUT_CUT#mmode== ${decay_mode_points[$dm_i]} \&\& mcharm == $CHARM_CHARM #g" `grep CUT_CUT  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        
                        sed -i "s#HHH#${energy_points[$ep_i]}--${cosp_mode[$cos_cout]}#g" `grep HHH  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        # sed -i "s#COSTHETA_Min#${costheta_min[$cos_cout]}#g" `grep COSTHETA_Min  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        # sed -i "s#COSTHETA_Max#${costheta_max[$cos_cout]}#g" `grep COSTHETA_Max  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        # sed -i "s#MOMENTUM_Min#${p_min[$cos_cout]}#g" `grep MOMENTUM_Min  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        # sed -i "s#MOMENTUM_Max#${p_max[$cos_cout]}#g" `grep MOMENTUM_Max  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        ...
                        then
                            sed -i "s#DDD#D^{+}#g" `grep DDD  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        fi

                        if [ ${decay_mode_points[$dm_i]} -lt 200 ]
                        then
                            sed -i "s#DDD#D^{0}#g" `grep DDD  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                        fi
                        ...
                        # sed -i "s#WEIGHT_Tree_WEIGHT_Tree#$weight_file#g" `grep WEIGHT_Tree_WEIGHT_Tree  -rl ${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp`
                       #- - 这里功能可以改进



                        echo "${jobDir}/all_mom_cos_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_costheta_momentum${cosp_mode[$cos_cout]}_file_${ncount}.cpp is finished "
