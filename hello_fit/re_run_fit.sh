...
do               
    ...
    do 
        ...
        if [ -d "$DataParentPath1/more_gaussian_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/failed" ]
        then
            echo "$DataParentPath1/more_gaussian_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/failed is ok"
        else
            echo "* * * * * * * * * * * *  $DataParentPath1/more_gaussian_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/failed is not exit ! ! !"
            let ep_i++
            continue
        fi

        cd ${DataParentPath1}/more_gaussian_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}/failed
