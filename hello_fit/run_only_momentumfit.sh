...
do               
    ...
    do 
        ...
        if [ -d "$DataParentPath1/only_momentum_differential_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[@]}" ]
        then
            echo "$DataParentPath1/only_momentum_differential_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[@]} is ok"
        else
            echo "* * * * * * * * * * * *  $DataParentPath1/only_momentum_differential_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[@]} is not exit ! ! !"
            let ep_i++
            continue
        fi

        cd ${DataParentPath1}/only_momentum_differential_fit/mode_${decay_mode_points[$dm_i]}/${energy_points[@]}
        ...
        do
            ...
            # echo "root -l -b -q ${file_number[$xy-1]}"
