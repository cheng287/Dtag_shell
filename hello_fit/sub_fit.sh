...
do               
    ...
    do 
        ...
        if [ -d "$DataParentPath1/fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}" ]
        then
            echo "$DataParentPath1/fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]} is ok"
        else
            echo "* * * * * * * * * * * *  $DataParentPath1/fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]} is not exit ! ! !"
            let ep_i++
            continue
        fi

        cd ${DataParentPath1}/fit/mode_${decay_mode_points[$dm_i]}/${energy_points[$ep_i]}
