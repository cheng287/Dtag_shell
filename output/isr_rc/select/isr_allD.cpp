...
double class_new_isr::Cross(double S)
{
    ...
    {		      
		...
		TString FileName("/besfs5/groups/psip/psipgroup/user/liucheng/cross_section/Rc_new_run/new_low_xyz/output/isr_rc/select/test_cross_section_allD.txt");		
...
void isr_allD()
{
	...
	for(int j = 0; j <row ; j++)
	{
		...
		std::ofstream outputFile("all_allD.txt", std::ios::app);
