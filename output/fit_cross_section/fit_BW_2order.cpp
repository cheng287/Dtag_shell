...
void fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
	...
	for(int i=0;i<n_fit;i++)
	{
		...
		f0 = pow((yy0[i]-FF_fit_2_order(nn, par))/eyy0[i],2);
		f += f0;
	}
...
void fit_BW_2order()
{
	...
	gMinuit->mnparm(3,    "a0",      	 rdm_param[3],  step[3],   -1.e6  ,   1.e5  ,   ierflg  );   // 
    gMinuit->mnparm(4,    "a1",          rdm_param[4],  step[4],   -1.e5  ,   1.e5 ,   ierflg  );	
	gMinuit->mnparm(5,    "a2",          rdm_param[5],  step[5],   -1.e5  ,   1.e5 ,   ierflg );      //  a1*x^2 +a0*x +a2
	gMinuit->mnparm(6,    "Phi",         rdm_param[6],  step[6],   -180.   ,   180.   ,   ierflg );  
    ...
    gMinuit->GetParameter(5,par1[5], epar1[5]);   // for a2 --  a0*x^2 +a1*x +a2
	gMinuit->GetParameter(6,par1[6], epar1[6]);  
	...
	for(int i = 1; i <= Nbin; i++)
	{
		...
		// bh = phsp2_poly1(bc, par2);   // one order bkg
		bh = phsp2_poly2(bc, par2);	    // 2 order bkg
	...
	TPad* pad_sub = new TPad("pad_sub", " ", 0.5, 0.17, 0.9, 0.6);
	...
	///-------------------
	// TPad* pad_sub2 = new TPad("pad_sub2", " ", 0.1, 0.17, 0.5, 0.6);
	// pad_sub2->SetFillStyle(4000); // 设置填充样式为透明
    // // pad_sub2->SetTopMargin(0.15);
    // pad_sub2->SetBottomMargin(0.15);
	// pad_sub2->SetLeftMargin(0.15);
	// // pad_sub2->SetLogy(true);
    // pad_sub2->Draw();
	///-------------------
	...
	///-------------------
	// pad_sub2->cd();
	// NameAxes(hbg, (char*)"#sqrt{#font[12]{s}} (GeV) ",(char*)" #sigma(e^{+}e^{-}#rightarrow D^{0}X + D^{+}X + D^{+}_{s}X) (pb)");
	// // hbg->GetXaxis()->SetRangeUser(0,3.7);
	// hbg->GetXaxis()->SetTitleSize(0.055);
    // hbg->GetYaxis()->SetTitleSize(0.055);
	// hbg->GetYaxis()->SetNdivisions(505);
    // hbg->GetXaxis()->SetNdivisions(505);
	// hbg->GetYaxis()->SetMaxDigits(4);    // 科学计数
    // hbg->GetYaxis()->SetTitleOffset(1.5);
    // hbg->GetXaxis()->SetTitleOffset(1.5);
    // hbg->GetXaxis()->SetTitleSize(0.04);
    // hbg->GetYaxis()->SetTitleSize(0.04);
	...
	c1->SaveAs("fit_bw_2order.png");
	// c1->SaveAs("fit_bw.eps");
