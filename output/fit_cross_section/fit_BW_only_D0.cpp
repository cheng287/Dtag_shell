...
void fit_BW_only_D0()
{
	...
	ifstream in1("/besfs5/groups/psip/psipgroup/user/liucheng/cross_section/Rc_new_run/4612to4946/output/isr_fit/fit_cross_section/obs_cross_section_mode0.txt");
	...
	gMinuit->mnparm(3,    "a1",      	 rdm_param[4],  step[4],   -1.e4  ,   1.e4  ,   ierflg  );
    gMinuit->mnparm(4,    "a0",          rdm_param[3],  step[3],   -1.e4  ,   1.5e4 ,   ierflg  );	
	gMinuit->mnparm(5,    "Phi",         rdm_param[5],  step[5],   -10.   ,   10.   ,   ierflg );  
	...
	int Nbin = 2000;
	TH2F * h_mode = new TH2F("h_mode","", Nbin, Xmin, Xmax, 200,2000,4500);
	...
	// TPad* pad_sub = new TPad("pad_sub", " ", 0.2, 0.45, 0.6, 0.8);
	...
	NameAxes(hbw1, (char*)"#sqrt{#font[12]{s}} (GeV) ",(char*)" #sigma(e^{+}e^{-}#rightarrow D^{0}X (pb)");
	SetStyle();
