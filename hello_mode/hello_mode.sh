...
#DataParentPath1=/scratchfs/bes/liucheng/cross_section/Rc_MC_eff_weight/mode_0/D0/ana/root
DataParentPath1=/scratchfs/bes/liucheng/cross_section/Rc_MC_eff_weight/mode_0/D0/ana/test/root
...
do
    ...
    #--do
        ...
        #------------------mode${decay_mode_points[$dm_i]}_momentum_costheta_
        ...
        do
            ...
            then 
                if [ -e ${jobDir}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp ] 
                then 
                    echo "Just remind: ${jobDir}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.txt already exists, and it will be update!"
                fi 
                
                sed -e s/XXX/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}/ ./infor_mode.head > ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp

            fi 

            if [[ `expr ${iline} % ${fileN}` -ne 0 && ${iline} -lt ${nline} ]] 
            then 
                echo  "    chaindata->Add("\"${file_number[$xy-1]}\" ") ;" >>  ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp
            fi 

            if [[ `expr ${iline} % ${fileN}` == 0 || ${iline} == ${nline} ]] 
            then 
                echo  "    chaindata->Add("\"${file_number[$xy-1]}\" ") ;" >>  ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp
                sed -e s/XXX/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}/ ./infor_mode.tail >> ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp

                sed -i "s/HHH_MODE/${decay_mode_points[$dm_i]}/g" `grep HHH_MODE  -rl ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp`
                #--sed -i "s/HHHRUN_LOW/${runnu_low[$rl_i]}/g"       `grep HHHRUN_LOW -rl ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp`
                #--sed -i "s/HHHRUN_HIGH/${runnu_high[$rh_i]}/g"     `grep HHHRUN_HIGH -rl ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp`


                echo ${jobDir}/mode_${decay_mode_points[$dm_i]}/${jobName}_mode${decay_mode_points[$dm_i]}_momentum_costheta_${ncount}.cpp is finished 
