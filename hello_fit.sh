...
do
    ...
    do
        ...
        if [ -d "./fit/mode_${decay_mode_points[$dm_i]}" ]                #* * * *  默认都是当前路径
        then
            echo "./fit/mode_${decay_mode_points[$dm_i]} is ok"
        else
            mkdir -p ./fit/mode_${decay_mode_points[$dm_i]}
        fi

        #* * *  if [ -d "./fit/mode_${decay_mode_points[$dm_i]}" ]
        #* * *  then
        #* * *      echo "fit/mode_${decay_mode_points[$dm_i]} is ok"
        #* * *  else
        #* * *      mkdir -p ./fit/mode_${decay_mode_points[$dm_i]}
        ...
        then
          ...
          echo "useage : ./hello_fit.sh jobName [jobN] [fileN] [jobDir]"
          ...
          echo "  "
          echo "SQUESTION: step one  : please check/modify the 'fitjob.head' "
          echo "           step two  : please check/modify the 'fitjob.tail' "
          echo "           step three: please check 'DataParentPath' in 'hello_fit.sh' correct or not"
          exit
        fi
        ...
        #DataParentPath1=/besfs5/groups/tauqcd/songwm/Rc_study/Rc_MC/D01/ana/root        #* * * * * * 路径，根据需要改变 (记住 ,这里的root 必须是通过hello_mode.sh 自动生成的 root 目录 ! ! ! ! !)
        
        
        
        DataParentPath1=$PWD/root                              #* * * * * 默认当前路径,不需要改变 ! ! ! 很重要，一般不注释它, 用别人跑的root 才注释这儿
        ...
        jobName=$1
        echo "jobName : ${jobName}"         #* * *  jobName=data, mc  等等, 必须和 hello_mode.sh 输入的 name 保持一致 ! ! ! ! ! !
        ...
        do
                ...
                #if [ `expr ${iline} % ${fileN}` == 1 ]   #* * 我注释它，，用了上面
                then 
                    if [ -e ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp ] 
                    then 
                       echo "Just remind: ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.txt already exists, and it will be update!"
                    fi 
                    
                    #* * * *  Ds 和 D0 ,D+- 的质量有区别，所以用不同的拟合脚本
                    if [ ${decay_mode_points[$dm_i]} -lt 400 ]
                    then
                        sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}/ ${jobDir}/fitjob.head >> ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp
                    else
                        sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}/ ${jobDir}/Ds_fitjob.head >> ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp
                    fi
                    
                    # sed -i "s/HHH_MODE/${decay_mode_points[$dm_i]}/g" `grep HHH_MODE -rl  ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp`
                    # sed -i "s/HHHRUN_LOW/${runnu_low[$rl_i]}/g"       `grep HHHRUN_LOW -rl ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp`
                ...
                then 
                    ...
                    echo  "    chaindata->Add("\"${file_number[$xy-1]}\" ") ;" >>  ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp
                fi 
                ...
                then 
                    ...
                    if [ ${decay_mode_points[$dm_i]} -lt 400 ]
                    then       
                        sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}/ ${jobDir}/fitjob.tail >> ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp
                    else
                        sed -e s/XXX/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}/ ${jobDir}/Ds_fitjob.tail >> ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp
                    fi
                    ...
                    echo ${jobDir}/fit/mode_${decay_mode_points[$dm_i]}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp is finished 
                    #boss -q ${jobName}_${ncount}.txt
                    #boss.condor ${jobName}_${ncount}.txt
                    mv ${jobDir}/mass_fit_${jobName}_${energy_points[$ep_i]}_mode${decay_mode_points[$dm_i]}_${ncount}.cpp ./fit/mode_${decay_mode_points[$dm_i]}
                    let i++
                    if [ ${i} -lt 10 ] 
                    then 
                        ijob=${i}
